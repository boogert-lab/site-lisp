...
;; Make Emacs tags expand environment variables because the standard
;; gis tags table refers to all the source files with $SMALLWORLD_GIS.
;; This is not implemented for XEmacs.

(eval-if-gnu-emacs
 (require 'etags)   ; Make sure the standard definition is loaded first
                    ; so that we can override it.
 (fset 'etags-file-of-tag-orig
       (symbol-function 'etags-file-of-tag))

 (defun etags-file-of-tag ()
   (save-excursion
     (search-backward "\f\n")
     (forward-char 2)
     (substitute-in-file-name   ; This is the function that expands env. vars.
